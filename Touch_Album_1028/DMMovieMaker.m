...
+ (void)makeMovieWithImages:(NSArray *)images withAudio:(NSURL *)audioURL {
    ...
    for (__weak UIImage *resizedimage in images) {
        if (adaptor.assetWriterInput.readyForMoreMediaData) {
            CMTime frameTime = CMTimeMake((int64_t)frameCount * fps * durationForEachImage, fps);
            // TODO: ここの調整(リサイズ後なのかそのままでいいのか)
             UIImage *resizedImage = [self cropRectImage:images];
            buffer = [self pixelBufferFromCGImage:resizedImage.CGImage withOriginImage:resizedImage];
            
            if (![adaptor appendPixelBuffer:buffer withPresentationTime:frameTime]) {
                NSLog(@"Failed to append buffer. [resizedImage : %@]", resizedImage);
            }
            if(buffer) {
                buffer = nil;
...
+ (void)exportDidFinish:(AVAssetExportSession *)session {
    if(session.status == AVAssetExportSessionStatusCompleted) {
        ...
        if ([library videoAtPathIsCompatibleWithSavedPhotosAlbum:outputURL] == true) {
            ...
            NSLog(@"%@", ([library videoAtPathIsCompatibleWithSavedPhotosAlbum:outputURL] ? @"true": @"false"));
        }
    }else {
